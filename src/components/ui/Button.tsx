...
export function Button({
  ...
  return (
    <button
      className={`inline-flex min-h-11 cursor-pointer touch-manipulation items-center justify-center gap-2 rounded-sm font-condensed font-semibold uppercase tracking-caps transition duration-200 ease-out disabled:cursor-not-allowed disabled:opacity-50 ${sizeClasses[size]} ${variantClasses[variant]} ${className}`}
      {...rest}
    >
      {children}
    </button>
